  ...
  pdf:
    ...
    include-in-header:
      text: |
        ...
        \usepackage[letterpaper,margin=1in,footskip=20pt]{geometry}
        % set table numbering
...
We implemented BERTopic [@grootendorst] on the entire set of 2224 relevant articles after cleaning the texts for stop words.
...
Here, we evaluated the same models (with the same settings and computing resources) as mentioned in the relevance classification section. All the zero-shot tests were done on the set of 100 articles that were manually labelled by both annotators. The fine-tuned was performed using the remaining 400 articles manually labelled by the first author. The prompts used for annotations are attached below:
...
df_gold <- read.csv(here::here("Framing/LLM/Predicted/Test/","llama3.1:8b-instruct-q8_0_zshot.csv"),
                    check.names = F)
...
models <- c("llama_llm_judge","llama_human_feedback","llama_decision","llama_confidence",
            "nb_imb", "bert_imb", "deberta_imb", "ensemble_dsl") 
...
names(models) <- c("Llama 3.1-8B-Instruct (8-bit quantized) with LLM as judge",
                   ...
                   "Naive Bayes with Class Imbalance management",
                   "BERT with Class Imbalance management",
                   "DeBERTa with Class Imbalance management",
                   "Design-based Supervised learning")
